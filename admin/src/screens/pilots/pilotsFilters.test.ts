...
describe('adres → filtr', () => {
  it('pusty adres daje filtr domyślny: wszystkie konta, alfabetycznie', () => {
    expect(filterFromParams(params(''))).toEqual(DEFAULT_PILOTS_FILTER);
  });
...
describe('filtr → adres', () => {
  ...
  it('podróż w obie strony zachowuje filtr', () => {
    const filter: PilotsFilter = { scope: 'inactive', search: 'MDB', sort: 'desc' };
  ...
  it('adres szuflady NIESIE zawężenie listy pod spodem', () => {
    const filter: PilotsFilter = { scope: 'panel', search: null, sort: 'asc' };
    expect(accountHref(filter, 'abc-123')).toBe('/piloci/abc-123?stan=panel');
    // Wariant „reset hasła" z mockupu A06a to ta sama szuflada z innym wejściem.
    expect(accountHref(filter, 'abc-123', 'haslo')).toBe('/piloci/abc-123?stan=panel&akcja=haslo');
    expect(newAccountHref(DEFAULT_PILOTS_FILTER)).toBe('/piloci/nowe');
  });
});
...
describe('filtr → parametry trasy', () => {
  it('„wszyscy" nie wysyła żadnego zawężenia', () => {
    expect(pilotListQuery(DEFAULT_PILOTS_FILTER)).toEqual({
      sort: 'asc',
  ...
  it('aktywni i nieaktywni jadą jako `active`, nie jako dwa różne parametry', () => {
    expect(pilotListQuery({ ...DEFAULT_PILOTS_FILTER, scope: 'active' })).toMatchObject({
      active: 'true',
    });
    expect(pilotListQuery({ ...DEFAULT_PILOTS_FILTER, scope: 'inactive' })).toMatchObject({
      active: 'false',
    });
  });
  ...
  it('wyszukiwanie jedzie jako `q`', () => {
    expect(pilotListQuery({ ...DEFAULT_PILOTS_FILTER, search: 'kza' })).toMatchObject({
      q: 'kza',
    });
  });
});
...
describe('czy filtr zawęża', () => {
  ...
  it('zakres i wyszukiwanie zawężają', () => {
    expect(isNarrowed({ ...DEFAULT_PILOTS_FILTER, scope: 'inactive' })).toBe(true);
    expect(isNarrowed({ ...DEFAULT_PILOTS_FILTER, search: 'x' })).toBe(true);
  });
});
