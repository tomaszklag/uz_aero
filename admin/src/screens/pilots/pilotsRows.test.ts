...
import { hasPanelRole, pilotsEmpty, pilotRows, roleBadge } from './pilotsRows';
...
describe('pusta lista', () => {
  it('mówi CO INNEGO przy zawężeniu niż bez niego', () => {
    expect(pilotsEmpty(true).title).not.toBe(pilotsEmpty(false).title);
    expect(pilotsEmpty(true).note).toContain('zawężenie');
    expect(pilotsEmpty(false).note).toContain('rejestracji');
  });
});
