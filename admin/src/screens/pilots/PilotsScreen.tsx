 ...
 * i dostępność akcji pochodzą z czystych modułów obok (`pilotsFilters`, `pilotsRows`,
 * `pilotsTiles`, `accountActions`), które mają testy w Node.
 *
...
import { activeAction, canManage, resetAction } from './accountActions';
import { pilotChips } from './pilotsChips';
import {
...
import { pilotsEmpty, pilotRows, type PilotRow } from './pilotsRows';
import { monthLabel, pilotTiles, roleSplit, roleSplitCaption } from './pilotsTiles';
...
export function PilotsScreen() {
  ...
  const empty = pilotsEmpty(isNarrowed(filter));
    ...
    <>
      ...
      {pilots.isPending ? null : pilots.isError ? (
        ...
        <DataTable
          ...
          onRowClick={(row) => {
            void navigate(accountHref(filter, row.id));
          }}
        />
      )}
      ...
      render: (row) => {
        ...
        return (
          <div className="row-actions">
            <LinkButton to={accountHref(filter, row.id)} variant="ghost" size="sm">
              Szczegóły
            </LinkButton>
            <LinkButton
              to={accountHref(filter, row.id, 'haslo')}
              variant="ghost"
              size="sm"
