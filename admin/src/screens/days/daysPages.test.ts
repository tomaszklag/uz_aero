...
describe('daysEmpty', () => {
  it('rozróżnia „pusty rejestr" od „nic w tym zawężeniu"', () => {
    ...
    expect(daysEmpty(true).title).not.toBe(daysEmpty(false).title);
    expect(daysEmpty(true).note).toContain('zawężona');
    expect(daysEmpty(false).note).toContain('session_claim');
  });
  ...
  it('ostrzega o pułapce sesji bez preflightu przy filtrze zakresu', () => {
    ...
    expect(daysEmpty(true).note).toContain('bez potwierdzenia przedlotowego');
  });
});
