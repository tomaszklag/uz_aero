...
describe('filterFromParams', () => {
  it('pusty adres daje filtr domyślny: bez zawężenia, najnowsze na górze', () => {
    expect(filterFromParams(params(''))).toEqual(DEFAULT_DAYS_FILTER);
  });
  ...
  it('wartości nieznane POMIJA zamiast odrzucać - adres z literówką pokazuje listę', () => {
    ...
    expect(filter).toEqual(DEFAULT_DAYS_FILTER);
  });
...
describe('sessionListQuery', () => {
  it('brak zawężenia to BRAK parametrów stanu, nie `status=all`', () => {
    const query = sessionListQuery(DEFAULT_DAYS_FILTER);

    expect(query).toEqual({ sort: 'desc', limit: DAYS_PAGE_LIMIT });
    expect(query).not.toHaveProperty('status');
    expect(query).not.toHaveProperty('flagged');
  });

  it('jeden chip stanu → właściwy parametr trasy, bo serwer ma na to trzy pola', () => {
    const of = (state: DaysFilter['state']) =>
      sessionListQuery({ ...DEFAULT_DAYS_FILTER, state });
  ...
  it('nie wysyła kursora - pierwszą stronę pobiera się bez niego', () => {
    ...
    expect(sessionListQuery(DEFAULT_DAYS_FILTER)).not.toHaveProperty('cursor');
  });
});
...
describe('sessionCountQuery', () => {
  ...
  it('pyta o jeden wiersz, bo liczy się wyłącznie `total`', () => {
    ...
    expect(sessionCountQuery(DEFAULT_DAYS_FILTER, 'open').limit).toBe(1);
  });
});

describe('isNarrowed', () => {
  it('rozróżnia „pusty rejestr" od „nic w tym filtrze"', () => {
    expect(isNarrowed(DEFAULT_DAYS_FILTER)).toBe(false);
    expect(isNarrowed({ ...DEFAULT_DAYS_FILTER, state: 'flagged' })).toBe(true);
    expect(isNarrowed({ ...DEFAULT_DAYS_FILTER, aircraftId: 'SP-KLM' })).toBe(true);
    expect(isNarrowed({ ...DEFAULT_DAYS_FILTER, from: '2026-07-25' })).toBe(true);
  });
  ...
  it('sam kierunek sortowania NIE jest zawężeniem', () => {
    ...
    expect(isNarrowed({ ...DEFAULT_DAYS_FILTER, sort: 'asc' })).toBe(false);
  });
});
