 ...
 * i podpis pochodzą z czystych modułów obok (`audytActions`, `audytFilters`,
 * `audytRows`, `audytDetails`, `audytPages`, `audytTiles`), które mają testy w Node.
 *
...
import { AUDIT_ACTION_META, AUDIT_ACTIONS, AUDIT_GROUPS } from './audytActions';
import { EMPTY_DETAILS_NOTE } from './audytDetails';
import {
  audytHref,
  auditListQuery,
  filterFromParams,
...
import { auditRows, type AuditRow } from './audytRows';
import { auditTiles, tileQueries } from './audytTiles';

export function AudytScreen() {
  ...
  const apply = (next: AudytFilter): void => setSearchParams(paramsFromFilter(next));

  const pages = auditPages(entries.data);
  const rows = auditRows(pages.items);
  const empty = audytEmpty(isNarrowed(filter));
                    ...
                    <tr key={code}>
                      ...
                      <td>
                        <div className="row-actions">
                          <LinkButton
                            to={audytHref({ ...filter, scope: { kind: 'action', code } })}
                            variant="ghost"
                            size="sm"
                          >
                            Pokaż
 ...
 */
function columns(filter: AudytFilter, apply: (next: AudytFilter) => void): Column<AuditRow>[] {
  return [
    {
      key: 'when',
      header: 'Czas · UTC',
    ...
    },
    {
      ...
      render: (row) => (
        <>
          ...
          <CellLink
            to={audytHref({ ...filter, actor: row.actor.pilotId })}
