...
import { DEFAULT_AUDYT_FILTER } from './audytFilters';
import { auditPages } from './audytPages';
import { auditTiles, tileQueries, utcDay } from './audytTiles';
...
describe('kafle dziennika audytu', () => {
  ...
  it('kafel „dziś" zawęża zakres do doby i ZOSTAWIA resztę filtra', () => {
    ...
    const { today } = tileQueries(
      { ...DEFAULT_AUDYT_FILTER, actor: 'TMK', from: '2026-01-01', to: '2026-12-31' },
      NOW,
    );
  ...
  it('kafel korekt podmienia zakres I akcję — i mówi o tym w przypisie', () => {
    const { corrections } = tileQueries(
      { ...DEFAULT_AUDYT_FILTER, scope: { kind: 'group', id: 'konta' }, actor: 'TMK' },
      NOW,
    );
  ...
  it('`limit: 1` — kafel pyta o `total`, nie o wiersze', () => {
    const queries = tileQueries(DEFAULT_AUDYT_FILTER, NOW);
  ...
  it('reguła „—" obowiązuje TAM, GDZIE KAFEL JEST WOŁANY — także przy błędzie pobrania', () => {
    ...
    //
    // Dlatego test składa oba moduły dokładnie tak, jak robi to `AudytScreen`:
