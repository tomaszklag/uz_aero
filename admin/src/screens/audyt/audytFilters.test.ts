...
describe('filtry dziennika audytu', () => {
  it('pusty adres daje filtr domyślny — bez zawężenia, najnowsze na górze', () => {
    expect(filterFromParams(params(''))).toEqual(DEFAULT_AUDYT_FILTER);
    expect(isNarrowed(DEFAULT_AUDYT_FILTER)).toBe(false);
  });
  ...
  it('filtr → adres pomija wartości domyślne (link da się przeczytać)', () => {
    expect(paramsFromFilter(DEFAULT_AUDYT_FILTER)).toEqual({});
    expect(audytHref(DEFAULT_AUDYT_FILTER)).toBe('/audyt');
  ...
  it('zapytanie trasy niesie wyłącznie ustawione pola', () => {
    expect(auditListQuery(DEFAULT_AUDYT_FILTER)).toEqual({ sort: 'desc', limit: 50 });

    const narrowed: AudytFilter = {
  ...
  it('każdy pojedynczy wymiar zawęża — także sam typ obiektu', () => {
    const only = (patch: Partial<AudytFilter>): boolean =>
      isNarrowed({ ...DEFAULT_AUDYT_FILTER, ...patch });
