...
export interface FlotaFilter {
  scope: FleetScope;
  /** Fragment rejestracji albo typu; `null` = bez wyszukiwania. */
  search: string | null;
}

export const DEFAULT_FLOTA_FILTER: FlotaFilter = { scope: 'all', search: null };
 ...
 */
export function filterFromParams(params: URLSearchParams): FlotaFilter {
  const scope = params.get('zakres');
  return {
    scope: isScope(scope) ? scope : DEFAULT_FLOTA_FILTER.scope,
 ...
 */
export function paramsFromFilter(filter: FlotaFilter): Record<string, string> {
  const params: Record<string, string> = {};
  if (filter.scope !== DEFAULT_FLOTA_FILTER.scope) params.zakres = filter.scope;
...
export function fleetListQuery(filter: FlotaFilter): FleetListQuery {
  return {
...
/** Czy filtr zawęża cokolwiek — pusta lista mówi wtedy co innego (patrz `flotaEmpty`). */
export function isNarrowed(filter: FlotaFilter): boolean {
  return filter.scope !== 'all' || filter.search != null;
}

/** Adres szuflady jednostki z ZACHOWANYM zawężeniem listy pod spodem. */
export function samolotHref(filter: FlotaFilter, id: string): string {
...
export function nowySamolotHref(filter: FlotaFilter): string {
  return samolotHref(filter, NEW_AIRCRAFT_SEGMENT);
}

/** Adres listy dni ZAWĘŻONEJ do tej jednostki. */
export function dniHref(aircraftId: string): string {
  return `/dni?samolot=${encodeURIComponent(aircraftId)}`;
}

/** Adres KARTY DNIA (`A02a`) — pełna strona, nie szuflada. */
export function dzienHref(sessionUuid: string): string {
...
export function dayLink(aircraftId: string, sessionUuid: string | null): DayLink {
  if (sessionUuid != null) return { to: dzienHref(sessionUuid), label: 'Otwarty dzień' };
  return { to: dniHref(aircraftId), label: 'Dni lotne' };
}
