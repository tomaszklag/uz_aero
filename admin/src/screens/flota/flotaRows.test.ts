...
describe('pusta lista', () => {
  it('mówi CO INNEGO przy zawężeniu niż bez niego', () => {
    expect(flotaEmpty(true).title).not.toBe(flotaEmpty(false).title);
    expect(flotaEmpty(true).note).toContain('zawężenie');
    expect(flotaEmpty(false).note).toContain('preflight');
  });
});
