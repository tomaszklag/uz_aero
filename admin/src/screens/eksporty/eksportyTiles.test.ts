...
describe('baner obcięcia listy', () => {
  ...
  it('mówi ile widać, ilu nie widać i że liczniki opisują CAŁY zakres', () => {
    // Wada, którą ten przypadek zamyka: `EKSPORTY_PAGE_LIMIT` nie było używane nigdzie,
...
describe('stan pusty', () => {
  it('rozróżnia „nikt nie latał" od „to zawężenie nic nie łapie"', () => {
    expect(eksportyEmpty(false).title).toContain('ŻADEN DZIEŃ');
    expect(eksportyEmpty(false).note).toContain('day_close');

    expect(eksportyEmpty(true).title).toContain('ZAWĘŻENIU');
    // Drugi przypadek wymaga podpowiedzi, jak z niego wyjść.
    expect(eksportyEmpty(true).note).toContain('Zdejmij chip');
  });
});
