...
describe('filtry monitora eksportu', () => {
  it('pusty adres daje filtr domyślny — bez ukrytego zakresu dat', () => {
    ...
    expect(filterFromParams(params(''))).toEqual(DEFAULT_EKSPORTY_FILTER);
    expect(isNarrowed(DEFAULT_EKSPORTY_FILTER)).toBe(false);
  });
  ...
  it('jest odwracalny, a wartości domyślne nie trafiają do adresu', () => {
    ...
    expect(paramsFromFilter(DEFAULT_EKSPORTY_FILTER)).toEqual({});
  });
  ...
  it('do trasy jedzie `state`, ale NIGDY `revised` — serwer takiego stanu nie zna', () => {
    ...
    const limit = { limit: EKSPORTY_PAGE_LIMIT };
    expect(exportListQuery({ ...DEFAULT_EKSPORTY_FILTER, scope: 'revised' })).toEqual(limit);
    expect(exportListQuery({ ...DEFAULT_EKSPORTY_FILTER, scope: 'blocked' })).toEqual({
      ...limit,
      state: 'blocked',
    });
    expect(exportListQuery(DEFAULT_EKSPORTY_FILTER)).toEqual(limit);
  });

  it('ZAWSZE wysyła limit — bezpiecznik ma być widoczny po obu stronach', () => {
    // Do 2026-08-01 `EKSPORTY_PAGE_LIMIT` nie było używane NIGDZIE, a jego docblock
    ...
    for (const scope of ['all', 'revised', 'missing'] as const) {
      expect(exportListQuery({ ...DEFAULT_EKSPORTY_FILTER, scope }).limit).toBe(
        EKSPORTY_PAGE_LIMIT,
      );
    }
  });
  ...
  it('buduje adres z zaznaczonym wierszem, zachowując zawężenie', () => {
    const filter: EksportyFilter = { ...DEFAULT_EKSPORTY_FILTER, scope: 'missing' };

    expect(eksportyHref(filter)).toBe('/eksporty?stan=missing');
    expect(eksportyHref(filter, 'sess-1')).toBe('/eksporty/sess-1?stan=missing');
    expect(eksportyHref(DEFAULT_EKSPORTY_FILTER, 'sess-1')).toBe('/eksporty/sess-1');
  });
});
