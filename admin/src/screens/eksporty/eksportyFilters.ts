 ...
 * `eksportyRows.ts`, bo serwer takiego parametru nie ma, a wymyślanie go tylko po to,
...
export interface EksportyFilter {
 ...
 */
export const DEFAULT_EKSPORTY_FILTER: EksportyFilter = {
  from: null,
  to: null,
  aircraftId: null,
  search: null,
 ...
 * (`eksportyTiles.truncationNotice`). Do 2026-08-01 stała nie była używana NIGDZIE:
 * `exportListQuery` nie wysyłało limitu, a docblock obiecywał komunikat, którego na
 * ekranie nie było.
 */
export const EKSPORTY_PAGE_LIMIT = 200;
 ...
 */
export function filterFromParams(params: URLSearchParams): EksportyFilter {
  ...
  return {
    ...
    scope: isScope(scope) ? scope : DEFAULT_EKSPORTY_FILTER.scope,
  };
}
 ...
 */
export function paramsFromFilter(filter: EksportyFilter): Record<string, string> {
  ...
  if (filter.scope !== DEFAULT_EKSPORTY_FILTER.scope) params.stan = filter.scope;
  return params;
}
 ...
 * o CAŁY zakres (bez `state`) i odsiewa rewizję 1 na wierszach (`eksportyRows.ts`) —
 ...
 */
export function exportListQuery(filter: EksportyFilter): ExportListQuery {
  return {
    ...
    ...(filter.scope === 'all' || filter.scope === 'revised'
      ? {}
      : { state: filter.scope }),
    limit: EKSPORTY_PAGE_LIMIT,
  };
}

/** Czy filtr cokolwiek zawęża — pusta lista mówi wtedy co innego (patrz `eksportyEmpty`). */
export function isNarrowed(filter: EksportyFilter): boolean {
  return (
    filter.from != null ||
    filter.to != null ||
...
export function eksportyHref(filter: EksportyFilter, sessionUuid?: string | null): string {
