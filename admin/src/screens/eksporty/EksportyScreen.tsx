 ...
 * i podpis pochodzą z czystych modułów obok (`eksportyFilters`, `eksportyRows`,
 * `eksportyStates`, `eksportyTiles`, `eksportyHistory`, `eksportySheet`), które mają
 * testy w Node.
 *
...
import {
  ...
  revisionEntries,
} from './eksportyHistory';
import { shortUuid } from '../flagi/flagRows';
import { exportRows, narrowToScope, type ExportRow } from './eksportyRows';
import { sheetLines, sheetWidth } from './eksportySheet';
import { retryLabel, retryMessage } from './eksportyStates';
import {
  eksportyEmpty,
  exportChips,
  exportTiles,
...
export function EksportyScreen() {
  ...
  const apply = (next: EksportyFilter): void => setSearchParams(paramsFromFilter(next));
  ...
  const open = (uuid: string | null): void => {
    void navigate(eksportyHref(filter, uuid === selected ? null : uuid));
  };

  const counts = page.data?.counts;
  const rows = narrowToScope(
    exportRows(page.data?.items ?? [], Date.now(), (uuid) => eksportyHref(filter, uuid)),
    filter.scope,
  );
  const empty = eksportyEmpty(isNarrowed(filter));
  ...
  return (
    <>
      <PageHead
        ...
        actions={
          <LinkButton to={eksportyHref({ ...filter, scope: 'missing' })} variant="ghost">
            Pokaż dni bez karty
          </LinkButton>
        }
      />
