...
import { actionsOfGroup, isAuditAction, isAuditGroup, type AuditGroupId } from './auditActions';
...
export function filterFromParams(params: URLSearchParams): AuditFilter {
  ...
  return {
    ...
    sort: isSort(sort) ? sort : DEFAULT_AUDIT_FILTER.sort,
  };
}
...
export function paramsFromFilter(filter: AuditFilter): Record<string, string> {
  ...
  if (filter.sort !== DEFAULT_AUDIT_FILTER.sort) params.sort = filter.sort;
  return params;
}
 ...
 */
export function auditHref(filter: AuditFilter): string {
