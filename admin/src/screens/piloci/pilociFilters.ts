...
import { DEFAULT_AUDYT_FILTER, audytHref } from '../audyt/audytFilters';
 ...
 */
export const DEFAULT_PILOCI_FILTER: PilociFilter = {
  scope: 'all',
  search: null,
  sort: 'asc',
};
 ...
 */
export const PILOCI_PAGE_LIMIT = 200;
...
export function filterFromParams(params: URLSearchParams): PilociFilter {
  const scope = params.get('stan');
  const sort = params.get('sort');

  return {
    scope: isScope(scope) ? scope : DEFAULT_PILOCI_FILTER.scope,
    search: trimmed(params.get('szukaj')),
    sort: isSort(sort) ? sort : DEFAULT_PILOCI_FILTER.sort,
  };
}
 ...
 */
export function paramsFromFilter(filter: PilociFilter): Record<string, string> {
  const params: Record<string, string> = {};
  if (filter.scope !== DEFAULT_PILOCI_FILTER.scope) params.stan = filter.scope;
  if (filter.search != null) params.szukaj = filter.search;
  if (filter.sort !== DEFAULT_PILOCI_FILTER.sort) params.sort = filter.sort;
  return params;
}

/** Adres listy dla danego filtra — JEDNO miejsce, w którym powstaje link do A06. */
export function pilociHref(filter: PilociFilter): string {
 ...
 */
export function kontoHref(filter: PilociFilter, id: string, action?: 'haslo'): string {
...
export function noweKontoHref(filter: PilociFilter): string {
  return kontoHref(filter, NEW_ACCOUNT_SEGMENT);
}
 ...
 */
export function pilotListQuery(filter: PilociFilter): PilotListQuery {
  return {
 ...
 *
 * Link składamy funkcją z ekranu audytu, a nie ręcznie: `targetHref`/`audytHref` są
...
export function accountsAuditHref(): string {
  return audytHref({ ...DEFAULT_AUDYT_FILTER, targetType: 'pilot' });
}

/** Czy filtr cokolwiek zawęża — pusta lista mówi wtedy co innego (`pilociRows`). */
export function isNarrowed(filter: PilociFilter): boolean {
