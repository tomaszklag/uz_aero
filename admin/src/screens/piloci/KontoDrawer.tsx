 ...
 * mieszkają w `kontoForm.ts` i `kontoActions.ts`, które mają testy w Node.
 */
...
import { drawerAccount } from './kontoAccount';
import {
  accountFailure,
  activeAction,
  activeChangeCopy,
...
/**
 * Szuflada jest KLUCZOWANA identyfikatorem konta w `PilociScreen` — więc zmiana konta
 ...
 */
export function KontoDrawer(props: KontoDrawerProps) {
...
function NewAccount({ capabilities, onClose }: KontoDrawerProps) {
  const [draft, setDraft] = useState<KontoDraft>({
    name: '',
    code: '',
    email: '',
    role: 'pilot',
...
function ExistingAccount({
  ...
  selfId,
  onClose,
}: KontoDrawerProps & { initial: PilotListItemDto }) {
  const [draft, setDraft] = useState<KontoDraft>(() => draftOf(initial));
...
function RoleChoice({
  ...
  role: KontoDraft['role'];
  disabled: boolean;
  onSelect: (role: KontoDraft['role']) => void;
}) {
  return (
