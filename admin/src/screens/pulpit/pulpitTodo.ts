...
import { dzienLink, eksportyBrakujaceHref, flagaHref } from './pulpitLinks';
...
function exportTask(row: ExportListItemDto, nowMs: number, windowMs: number): TodoTask {
  ...
  return {
    ...
    blocking: true,
    to: eksportyBrakujaceHref(row.sessionUuid),
  };
}
