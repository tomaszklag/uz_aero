...
const byKey = (key: string, data: Parameters<typeof pulpitTiles>[0]) => {
  const tile = pulpitTiles(data).find((t) => t.key === key);
...
describe('brak odpowiedzi z serwera', () => {
  ...
  it('przejścia ZOSTAJĄ czynne — lista docelowa może się pobrać, choć pulpit nie', () => {
    for (const tile of pulpitTiles(null)) {
