...
import type { Freshness } from '../flota/flotaRows';
import { dzienLink, samolotLink } from './pulpitLinks';
...
export function fleetNowRows(
  ...
  return items.map((item) => {
    ...
    return {
      ...
      to: engine == null ? samolotLink(aircraft.id) : dzienLink(engine.sessionUuid),
    };
  });
}
