...
import { SUSPICIOUS_AFTER_MS, ciszaView, isQuiet } from './pulpitCisza';
...
describe('cisza PODEJRZANA — cztery warunki, każdy osobno wystarcza', () => {
  ...
  it('3a. dokładnie NA progu cisza jest jeszcze spodziewana — granica jest ostra', () => {
    ...
    expect(ciszaView(data).verdict).toBe('expected');
  });

  it('4. karta dnia bez arkusza — i osobno: karta zablokowana flagą', () => {
    const missing = quiet();
    missing.counts.exports.missing = 1;
    expect(ciszaView(missing).reasons.map((r) => r.key)).toContain('karta-bez-arkusza');

    const blocked = quiet();
    blocked.counts.exports.blocked = 2;
    expect(ciszaView(blocked).reasons.map((r) => r.key)).toContain('karta-zablokowana');
  });

  it('nagłówek MÓWI, że pustka nie tłumaczy się sama', () => {
    const data = quiet();
    data.counts.exports.missing = 1;
    expect(ciszaView(data).headline).toContain('NIE jest zgodna z projektem');
  });
});
