 ...
 *     wprost (`korektaResult.ts`), bo powtórzona korekta dopisze drugie zdarzenie.
 *
 * Ekran jest `.tsx` bez arytmetyki i bez decyzji o treści: walidacja, napisy, wiersze
 * i komunikaty mieszkają w czystych modułach obok (`korektaDraft`, `korektaTarget`,
 * `korektaImpact`, `korektaResult`), które mają testy w Node.
 */
...
import { impactRows } from './korektaImpact';
import { correctionFailure, correctionOutcome, violationMessages } from './korektaResult';
import { targetRows } from './korektaTarget';

interface KorektaDrawerProps {
  sessionUuid: string;
  targetUuid: string;
...
function CorrectionForm({
  ...
  pilot,
  onClose,
}: KorektaDrawerProps & { entry: TimelineEntryDto }) {
