 ...
 * ostatniego dnia lotnego zamiast dzisiejszych zer. Werdykt liczy `dashboardQuiet.ts`.
 *
...
export function DashboardScreen() {
  ...
  const quietDetails = data == null ? null : quietView(data);
  ...
  return (
    <>
        ...
        actions={
          <>
            ...
            {data != null && data.counts.openFlags > 0 ? (
              <LinkButton to={flagsHref()} variant="primary">
                {`${data.counts.openFlags} flag do wyjaśnienia`}
              </LinkButton>
            ) : (
              <LinkButton to={openDaysHref()} variant="ghost">
                Dni bez zamknięcia
              </LinkButton>
            )}
          </>
      ...
      ) : quietDetails != null && quiet ? (
        <Banner tone={quietDetails.verdict === 'expected' ? 'ok' : 'warn'}>
          <b>{`${quietDetails.label}.`}</b> {quietDetails.headline}
        </Banner>
      ) : (
        <Banner tone="status">
      ...
      {quietDetails == null || !quiet ? null : (
        <Card
          title="Cisza spodziewana czy podejrzana"
          actions={
            <>
              <Pill tone={quietDetails.verdict === 'expected' ? 'green' : 'amber'} dot>
                {quietDetails.verdict === 'expected' ? 'spodziewana' : 'do sprawdzenia'}
              </Pill>
              ...
              {quietDetails.reasons.length === 0 ? (
                ...
                <Banner tone="warn">
                  ...
                  <ul className="reason-list">
                    {quietDetails.reasons.map((reason) => (
      ...
      <Columns wide>
        <div className="cols-stack">
          ...
          <Card
            title="Wymaga uwagi"
            actions={
              <LinkButton to={flagsHref()} variant="ghost" size="sm">
                Wszystkie flagi
              </LinkButton>
            }
          >
