 ...
 */
export const DEFAULT_DNI_FILTER: DniFilter = {
  from: null,
  to: null,
  aircraftId: null,
  pilotId: null,
 ...
 */
export const DNI_PAGE_LIMIT = 50;
 ...
 */
export function filterFromParams(params: URLSearchParams): DniFilter {
  ...
  return {
    ...
    state: isState(state) ? state : DEFAULT_DNI_FILTER.state,
    operation: isOperationType(operation) ? operation : null,
    sort: isSort(sort) ? sort : DEFAULT_DNI_FILTER.sort,
  };
}
 ...
 */
export function paramsFromFilter(filter: DniFilter): Record<string, string> {
  ...
  if (filter.state !== DEFAULT_DNI_FILTER.state) params.stan = filter.state;
  if (filter.operation != null) params.operacja = filter.operation;
  if (filter.sort !== DEFAULT_DNI_FILTER.sort) params.sort = filter.sort;
  return params;
}
 ...
 */
export function sessionListQuery(filter: DniFilter): SessionListQuery {
  return {
 ...
 */
export function sessionCountQuery(filter: DniFilter, state: StateFilter): SessionListQuery {
  // `limit: 1`, bo trasa wymaga liczby dodatniej — liczy się wyłącznie `total`.
  return { ...sessionListQuery({ ...filter, state }), limit: 1 };
}

/** Czy filtr zawęża cokolwiek — pusta lista mówi wtedy co innego (patrz `dniEmpty`). */
export function isNarrowed(filter: DniFilter): boolean {
  return (
    filter.from != null ||
    filter.to != null ||
