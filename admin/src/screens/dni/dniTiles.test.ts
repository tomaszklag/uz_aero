...
import { dayPages } from './dniPages';
import { dniTiles, type DniCounts } from './dniTiles';

/** Same braki — punkt wyjścia, w którym każdy kafel podmieniamy pojedynczo. */
const NONE: DniCounts = { total: undefined, open: undefined, flagged: undefined, exported: undefined };
...
describe('kafle listy dni', () => {
  it('BRAK odpowiedzi to „—", nie zero — zero jest twierdzeniem o świecie', () => {
    const tiles = dniTiles(NONE, false);
  ...
  it('reguła „—" obowiązuje TAM, GDZIE KAFEL JEST WOŁANY — także przy błędzie pobrania', () => {
    ...
    //
    // Dlatego test składa oba moduły dokładnie tak, jak robi to `DniScreen`:
    // `dayPages(days.data)` → `dniTiles({ total: pages.total, … })`. Osobno każdy
    // z nich przechodził.
    const noResponse = dayPages(undefined);

    expect(noResponse.total).toBeNull();
    expect(dniTiles({ ...NONE, total: noResponse.total }, false)[0]!.value).toBe('—');

    // Pusty rejestr NADAL pokazuje zero — to jest odpowiedź serwera, a nie jej brak.
    const empty = dayPages([page(0, 0)]);
    expect(dniTiles({ ...NONE, total: empty.total }, false)[0]!.value).toBe(0);
  });

  it('zero jest zerem, a liczba dodatnia zapala ton', () => {
    const tiles = dniTiles({ total: 128, open: 0, flagged: 3, exported: 91 }, true);
  ...
  it('zero flag świeci na zielono, ale brak liczby flag NIE świeci wcale', () => {
    ...
    expect(dniTiles({ ...NONE, flagged: 0 }, false)[2]!.tone).toBe('green');
    expect(dniTiles({ ...NONE, flagged: null }, false)[2]!.tone).toBeUndefined();
  });

  it('przypis pierwszego kafla mówi, czy liczba dotyczy filtra, czy całości', () => {
    expect(dniTiles(NONE, true)[0]!.note).toContain('filtr z adresu');
    expect(dniTiles(NONE, false)[0]!.note).toContain('Wszystkie sesje w rejestrze');
  });
});
