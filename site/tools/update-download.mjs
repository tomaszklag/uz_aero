 ...
 *         (tag android-v<wersja>-<build>, plik ninerdeck.apk); strona kieruje na trwały adres
 *         https://github.com/tomaszklag/uz_aero/releases/latest/download/ninerdeck.apk.
 ...
 * `--title Ninerdeck 1.1.0 (build 2)` rozpada się na kawałki, a nawiasy powłoka bierze
...
if (flag('release')) {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'ninerdeck-apk-'));
  const apk = join(dir, 'ninerdeck.apk');
  ...
  if (exists) {
    ...
    run('gh', ['release', 'create', tag, apk, '--repo', RELEASE_REPO, '--title', `Ninerdeck ${build.version} (build ${build.number}) - Android`, '--notes', `Build produkcyjny EAS z ${build.date.toISOString().slice(0, 10)}. Plik APK do instalacji na Androidzie.`]);
  }
