...
Zakres wybierasz z listy **Zestaw uprawnień**: **Pilot** (sama aplikacja), **Akceptujący** (zgoda na cudze rezerwacje z telefonu, bez panelu), **Koordynator lotów** (panel, cudze rezerwacje i ich akceptacja), **Technik** (panel i flota) albo **Administrator** (wszystko, co klub może nadać). **Pokaż zdolności** rozpisuje zestaw na pojedyncze pozycje; zaznaczenie albo odznaczenie którejkolwiek zmienia nazwę na **Własny zakres**. Więcej o tym, co daje każda zdolność: [konta i bezpieczeństwo](konta-i-bezpieczenstwo).
...
- **Zakres wyznacza, gdzie się pracuje.** Bez zdolności **Wejście do panelu** członek ma samą aplikację - także akceptujący, który rozstrzyga swój krok akceptacji z telefonu. Z nią loguje się do panelu tym samym kontem, którym wchodzi do aplikacji. Kto spróbuje wejść do panelu bez tej zdolności, dostaje komunikat, a nie awarię.
