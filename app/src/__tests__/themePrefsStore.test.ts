...
describe('ThemePrefsStore', () => {
  it('rekordy żyją per pilot - przelogowanie na wspólnym telefonie zmienia motyw', async () => {
    ...
    await store.write('AKO', { theme: 'paper', updatedAt: 1000, dirty: true });
    await store.write('BNO', { theme: 'amber', updatedAt: 2000, dirty: false });

    expect(await store.read('AKO')).toEqual({ theme: 'paper', updatedAt: 1000, dirty: true });
    expect(await store.read('BNO')).toEqual({ theme: 'amber', updatedAt: 2000, dirty: false });
  });

  it('pilot bez rekordu i bez starego klucza = null (ThemeProvider zostaje przy Night)', async () => {
    expect(await new ThemePrefsStore(new MemoryKv()).read('AKO')).toBeNull();
  });
  ...
  it('migracja: stary klucz per telefon staje się punktem startowym pilota (updatedAt=0, bez dirty)', async () => {
    ...
    expect(kv.data.get(`${LEGACY_THEME_KEY}.AKO`)).toBe(JSON.stringify(adopted));
    // …a stary klucz zostaje: następny pilot tego telefonu dziedziczy ten sam punkt startowy.
    expect(await store.read('BNO')).toEqual(adopted);
  ...
  it('własny rekord pilota ma pierwszeństwo przed starym kluczem', async () => {
    ...
    const store = new ThemePrefsStore(kv);
    await store.write('AKO', { theme: 'solar', updatedAt: 500, dirty: false });

    expect(await store.read('AKO')).toEqual({ theme: 'solar', updatedAt: 500, dirty: false });
  });
  ...
  it('zepsuty zapis (nie-JSON, zły kształt) = brak rekordu, nie wyjątek przy starcie', async () => {
    ...
    kv.data.set(`${LEGACY_THEME_KEY}.AKO`, 'night'); // goły tekst pod NOWYM kluczem - nie nasz format
    expect(await store.read('AKO')).toBeNull();

    kv.data.set(`${LEGACY_THEME_KEY}.AKO`, JSON.stringify({ theme: 'night' })); // bez metadanych LWW
    expect(await store.read('AKO')).toBeNull();
  });
});
